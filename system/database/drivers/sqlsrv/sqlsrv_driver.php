...
protected function _limit($sql, $limit, $offset)
    {
        //return preg_replace('/(^\SELECT (DISTINCT)?)/i','\\1 TOP '.($limit + $offset).' ', $sql); //original
        if($offset === FALSE) {
            ...
            if(version_compare($this->version(), '11', '>=')) {
                ...
                {
                    $message = "MSSQL must have an ORDER BY when limiting by offset.";
                    show_error($message);
                ...
                // need to escape regex special characters in $orderBy by using preg_quote()
                $sql = preg_replace('/(\\'. preg_quote($orderBy) .'\n?)/i','', $sql); // remove orderBy from original SQL as the OVER statement uses it
